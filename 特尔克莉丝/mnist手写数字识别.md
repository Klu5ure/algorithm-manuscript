...
![1714009151438](Z:\技术学习资料\python人工智能\lzj\assets\1714009151438.png)
...
def test_model():
    ...
    new_net = Net()
    new_net.load_state_dict(torch.load('mnist_net.pth'))
    for (n, (x, _)) in enumerate(test_data):
        if n > 0:
            break
        predict = torch.argmax(new_net.forward(x[0].view(-1, 28*28)))
        plt.figure(n)
        plt.imshow(x[0].view(28, 28), cmap='gray')  # 显示图像，使用灰度色图
        plt.title("prediction: " + str(int(predict)))
    plt.show()  # 显示图像
